...
export default function ProjectGalleryModal({
  ...
  // Reset zoom states when modal closes
  useEffect(() => {
    if (!isOpen) {
      setZoomedStates({ before: false, after: false });
  ...
  const navigateMedia = (type, direction) => {
    if (type === "before") {
      ...
      setZoomedStates((prev) => ({ ...prev, before: false }));
    } else {
      const mediaLength = afterMedia.length;
      setCurrentAfterIndex((prev) =>
        direction === "next"
      ...
      setZoomedStates((prev) => ({ ...prev, after: false }));
    }
  };

  const toggleZoom = (type) => {
    setZoomedStates((prev) => ({ ...prev, [type]: !prev[type] }));
  };

  const renderMedia = (media, type) => {
    const isZoomed = zoomedStates[type];
      ...
      return (
        <div
          className={`relative w-full ${
            isZoomed
              ? "h-[80vh] cursor-zoom-out overflow-auto"
              : "h-[500px] cursor-zoom-in"
          }`}
          onClick={() => toggleZoom(type)}
        >
          ...
          <Image
            ...
            unoptimized={isZoomed} // Disable optimizations when zoomed
          />
        </div>
      );
    }
          ...
          <div
            ...
            onClick={() => {
              type === "before"
                ? setCurrentBeforeIndex(idx)
                : setCurrentAfterIndex(idx);
              setZoomedStates((prev) => ({ ...prev, [type]: false }));
            }}
          >
            {media.type === "video" ? (
              <div className="relative h-full w-full bg-gray-200">
      ...
      <DialogContent className="max-w-5xl sm:max-w-7xl p-0 overflow-auto h-screen">
        ...
        <Tabs defaultValue="before" className="w-full mt-4">
          ...
          {beforeMedia.length > 0 && (
            <TabsContent value="before" className="mt-0">
              <div className="relative">
                {renderMedia(beforeMedia[currentBeforeIndex], "before")}
          ...
          <TabsContent value="after" className="mt-0">
            <div className="relative">
              {renderMedia(afterMedia[currentAfterIndex], "after")}
